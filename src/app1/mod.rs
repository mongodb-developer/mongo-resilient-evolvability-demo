...
pub async fn app1_main(url: &str) -> Result<(), Box<dyn Error + Send + Sync>> {
    ...
    let update_item = warp::put()
        .and(api_path_json_capture_filter_chain.clone())
        .and_then(update_book_list);
    // DELETE: HTTP DELETE filter chain
    let delete_item = warp::delete()
        .and(api_path_json_capture_filter_chain.clone())
        .and_then(delete_book_list);
    ...
    println!("- Eg: http://{}:{}/{}/{}?title=The%20Day%20of%20the%20Triffids&author=John%20Wyndham",
...
//
async fn insert_book_list(book_payload: BookPayload, booksmgr: BooksMgr)
                          -> Result<impl warp::Reply, warp::Rejection> {
    match booksmgr.db_insert_book(&mut book_payload_to_book(&book_payload)).await {
        Ok(_) => Ok(warp::reply::with_status("Inserted new book into the book list",
...
//
async fn update_book_list(book_payload: BookPayload, booksmgr: BooksMgr)
                          -> Result<impl warp::Reply, warp::Rejection> {
    match booksmgr.db_update_book(&book_payload_to_book(&book_payload)).await {
        Ok(_) => Ok(warp::reply::with_status("Incremented book amount in the book list",
                    http::StatusCode::CREATED)),
        Err(e) => {
...
//
async fn get_books_list(book_payload: BookPayload, booksmgr: BooksMgr)
                        -> Result<impl warp::Reply, warp::Rejection> {
...
//
async fn delete_book_list(book_payload: BookPayload, booksmgr: BooksMgr)
                          -> Result<impl warp::Reply, warp::Rejection> {
    match booksmgr.db_delete_book(&book_payload_to_book(&book_payload)).await {
        Ok(_) => Ok(warp::reply::with_status("Removed book from books list",
                    http::StatusCode::OK)),
        Err(e) => {
            eprintln!("Error deleting data: {}", e.to_string());
            Err(warp::reject())
...
//
fn books_to_books_payload(books: &[Book])
                          -> Vec<BookPayload> {
